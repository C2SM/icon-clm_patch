...
then
  ...
  if [ ${NUM_RESTART_PROCS} -eq 0 ]; then
    ...
    resdatafile=/capstor/store/cscs/c2sm/c2sme/ICON-CLM/multifile_restart_ATMO_19500101T000000Z.mfr
...
&nwp_tuning_nml
  tune_albedo_wso = 0,0
  tune_gkwake   = 0.65 !Merrit-orography setting else 1.5
  tune_gfrcrit  = 0.35 !Merrit-orography setting else 0.425
  tune_gkdrag   = 0.08 !Merrit-orography setting else 0.08
  tune_dust_abs = 1.
  tune_zvz0i    = 0.85
  ...
  lcalib_clcov =.FALSE.
  itune_gust_diag = 2 !Merrit-orography setting
  tune_grcrit = 0.5   !Merrit-orography setting
  tune_minsso = 1.0   !Merrit-orography setting
  tune_blockred = 1.5 !Merrit-orography setting
/
&turbdiff_nml
  tkhmin  = 0.6
  tkhmin_strat = 1.0
